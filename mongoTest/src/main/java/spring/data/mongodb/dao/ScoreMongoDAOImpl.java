...
public class ScoreMongoDAOImpl implements ScoreMongoDAO {
	...
	public List<ScoreDTO> findCriteria(String key, String value) {
		System.out.println("조회된key: "+key);
		String[] data = key.split(",");
		/*
		 * Query 클래스의 addCriteria메소드와
	...
	//mongodb가 json의 형태로 모든 작업을 처리하므로 key,value로 조건을 정의
	...
	public void update(ScoreDTO document) {
		//수정할 document에 조건을 적용 - rdbms의 where 절
	...
	public List<ScoreDTO> findAll(int pageNo) {
		...
		//spring-data내부에서 페이징 처리가 된 객체를 전달 받을 수 있다.
		//페이징 처리를 내부에서 할 수 있도록 필요한 정보를 findAll메소드에서 Pageable타입의 매개변수로 전달 받는다.
		//[정보 전달]
