...
public class AirMultipleReducer extends
		...
		protected void reduce(Text key, Iterable<IntWritable> values,
			...
			if(data[0].equals("Arrival")) {
			...
			}else if(data[0].equals("Departure")) {
				int sum = 0;
				for(IntWritable value:values) {
					sum=sum+value.get();
				}
			...
			}else if(data[0].equals("Normal1")){
				int sum = 0;
				for(IntWritable value:values) {
					sum=sum+value.get();
				}
				resultVal.set(sum);
				multiOut.write("Normal1", resultKey, resultVal);
			}else {
				int sum = 0;
				for(IntWritable value:values) {
					sum=sum+value.get();
				...
				resultVal.set(sum);
				multiOut.write("Normal2", resultKey, resultVal);
			}
		}
}
